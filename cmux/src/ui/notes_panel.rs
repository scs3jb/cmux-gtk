...
//!
//! All notes are stored client-side under `<data>/cmux/notes/` in a mirror tree
...
/// Default (global) notes file: a configured path, else `<data>/cmux/notes.md`.
...
/// `<data>/cmux` — the base directory for cmux client-side data.
...
/// Root of the per-context notes mirror tree: `<data>/cmux/notes/`.
...
mod tests {
    ...
    fn git_root_walks_up_to_dot_git() {
        let tmp = std::env::temp_dir().join(format!("cmux-notes-test-{}", std::process::id()));
    ...
    fn save_or_delete_writes_then_removes() {
        let tmp = std::env::temp_dir().join(format!("cmux-note-sd-{}.md", std::process::id()));
