...
pub fn load(workspace_dir: &str) -> Vec<CommandEntry> {
    ...
    if !workspace_dir.is_empty() {
        paths.push(Path::new(workspace_dir).join(".cmux/cmux.json"));
        paths.push(Path::new(workspace_dir).join("cmux.json"));
    }
    paths.push(super::config_dir().join("cmux.json"));
    ...
    for path in paths {
        ...
        let Ok(parsed) = serde_json::from_str::<CmuxJson>(&content) else {
            continue;
        };
...
mod tests {
    ...
    fn parses_simple_command() {
        ...
        let parsed: CmuxJson = serde_json::from_str(json).unwrap();
